...
<body>
  ...
  <div class="login-html">
    ...
    <div class="login-form">
      <form class="sign-in-htm" action="./api/user/capiusu.php?usuario=<?php echo $usuario; ?>&rol=<?php echo $rol; ?>" method="POST">
        <div class="group">
        ...
        <div class="group">
          ...
          <?php
                     $sentencia ="SELECT * FROM capacitaciones WHERE DATEDIFF(fechaf, fechai) > 0";
                     $resultado = $conexion->query($sentencia) or die ("Error al actualizar datos".mysqli_error($conexion));
                     while ($fila =$resultado ->fetch_assoc()) {
      ...
      <div class="group">
          <button class="button" style="position: absolute;top: 80%;" onclick="location.href='capacitacionesusu.php?usuario=<?php echo $usuario; ?>&rol=<?php echo $rol; ?>'">Volver</button>
      </div>
      <div class="group">
          <button class="button" style="position: absolute;top: 110%;" onclick="location.href='./api/user/cerrar.php?metodo=get;'">Cerrar sesión</button>
        </div>
    </div>
  </div>
</div>
